...
# Use filesystem for local development, memory for Vercel deployment
is_vercel = os.environ.get('VERCEL')
if is_vercel:
    app.config['SESSION_TYPE'] = 'filesystem' 
    # Create a temp directory in /tmp which is writable in Vercel
...
def player_obfuscated(auth_param):
    """Serve the video player page with obfuscated URL"""
    video_index = decode_obfuscated_url(auth_param)
    
    if video_index is None:
        flash('Invalid or expired video link', 'error')
...
def player(video_index):
    """Redirect to obfuscated URL"""
    # Generate obfuscated URL
    auth_param = generate_obfuscated_url(video_index, session.get('user_id'))
    
    # Redirect to obfuscated URL
    return redirect(url_for('player_obfuscated', auth_param=auth_param))
...
def share_links_admin():
    ...
    try:
        ...
        return render_template('share_links_admin.html', share_links=share_links)
    except Exception as e:
...
def disable_all_share_links():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'All share links have been disabled'})
    except Exception as e:
...
def delete_all_share_links():
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'All share links have been deleted'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
...
def get_batch_info(batch_id):
    ...
    try:
        ...
        # Add thumbnail from BATCH_THUMBNAILS
        if BATCH_THUMBNAILS:
            if batch_id in BATCH_THUMBNAILS:
                batch_data['thumbnail'] = BATCH_THUMBNAILS[batch_id]
            else:
                batch_data['thumbnail'] = BATCH_THUMBNAILS.get('default', '')
        
        # Count video and PDF items
        video_count = 0
        pdf_count = 0
        
        # Handle both new format (direct subjects) and old format (with batch_info)
        if 'name' in batch_data and 'subjects' in batch_data:
            # Direct format - from tk_converter
            ...
            # Old format with batch_info
            for subject_key, subject_data in batch_data.get('subjects', {}).items():
                if not subject_data:
                    continue
                
                # Handle old format where subjects contain chapter dictionaries
                if isinstance(subject_data, dict) and not ('name' in subject_data and 'content' in subject_data):
                    for chapter_key, chapter_items in subject_data.items():
                        if not isinstance(chapter_items, list):
                            continue
                        for item in chapter_items:
                            if isinstance(item, dict):
                                item_type = item.get('type', item.get('content_type', ''))
                                if item_type == 'video':
                                    video_count += 1
                                elif item_type == 'pdf':
                                    pdf_count += 1
                # Handle new format where subjects have content array
                elif isinstance(subject_data, dict) and 'content' in subject_data:
                    content_items = subject_data.get('content', [])
            for item in content_items:
                if item.get('type') == 'video':
                    video_count += 1
        ...
        batch_data['video_count'] = video_count
        batch_data['pdf_count'] = pdf_count
        
        # Ensure batch_info exists for template compatibility
        if 'batch_info' not in batch_data:
            batch_data['batch_info'] = {
                'name': batch_data.get('name', 'Untitled Batch'),
                'type': batch_data.get('type', 'Regular'),
        ...
        elif 'batch_info' in batch_data:
            # Ensure top-level attributes also exist
            if 'name' not in batch_data:
                batch_data['name'] = batch_data['batch_info'].get('name', 'Untitled Batch')
            if 'type' not in batch_data:
                batch_data['type'] = batch_data['batch_info'].get('type', 'Regular')
            if 'year' not in batch_data:
                batch_data['year'] = batch_data['batch_info'].get('year', '')
            if 'created_at' not in batch_data:
                batch_data['created_at'] = batch_data['batch_info'].get('created_at', '')
...
# Start auto-ping mechanism on Render (not on Vercel)
if is_render:
    setup_auto_ping()
    print("Running on Render: Auto-ping mechanism initialized")
...
def health_check():
    ...
    try:
        ...
        mem_usage = f"{mem_percent}% ({mem.used // (1024*1024)}MB / {mem.total // (1024*1024)}MB)"
    ...
    # Get last ping times
    last_pings = getattr(app, 'last_ping_time', [])
    ping_history = '<br>'.join(last_pings) if last_pings else 'No pings recorded yet'
    ...
    <head>
        ...
        <title>VidH Health Status</title>
        <style>
            body {{
    ...
    <body>
        ...
        <div class="card">
            ...
            <div class="metric">
                ...
                <span class="value">{mem_usage}</span>
            </div>
            <div class="metric">
            ...
            <div class="metric">
                ...
                <span class="value">{load_avg[0]:.2f}, {load_avg[1]:.2f}, {load_avg[2]:.2f}</span>
            </div>
            <div class="metric">
                <span class="label">Active Streams:</span>
                <span class="value">{active_streams_count}</span>
            </div>
            <div class="metric">
                <span class="label">Active Sessions:</span>
                <span class="value">{active_sessions_count}</span>
            </div>
        </div>
        <div class="card">
            <p><strong>Auto-ping:</strong> The server is configured to prevent Render free tier sleep.</p>
            <p><strong>Last ping:</strong> <span id="last-ping">Just now</span></p>
            <div class="ping-history">
                <strong>Ping History:</strong><br>
                {ping_history}
            </div>
        </div>
        
        <a href="/health" class="refresh">Refresh</a>
        
        <script>
            // This will update the "Last ping" text periodically
            setInterval(() => {{
...
# Auto-ping mechanism to prevent Render free tier sleep
def setup_auto_ping():
    """Setup a background thread to ping the application periodically"""
    def ping_app():
        ping_interval = 10 * 60  # 10 minutes
        render_sleep_time = 15 * 60  # Render free tier sleeps after 15 minutes
        
        app_url = os.environ.get('APP_URL')
        if not app_url:
            print("AUTO-PING: No APP_URL provided, using default localhost")
        ...
        while True:
            try:
                print(f"AUTO-PING: Sending ping to {ping_url}")
                response = requests.get(ping_url, timeout=10)
                print(f"AUTO-PING: Received response: {response.status_code}")
            except Exception as e:
                print(f"AUTO-PING: Error during ping: {e}")
            
            # Sleep for ping_interval seconds (less than render_sleep_time)
            time.sleep(ping_interval)
    
    # Start ping thread
    ping_thread = threading.Thread(target=ping_app, daemon=True)
    ping_thread.start()
    print("AUTO-PING: Background thread started to prevent Render sleep")
...
if __name__ == '__main__':
    # Start the auto-ping mechanism when running directly (not in Vercel)
    setup_auto_ping()
    app.run(debug=True, threaded=True)

# Simple ping endpoint for external monitoring tools
@app.route('/ping')
def ping():
    """Simple ping endpoint that responds with 'pong' to keep the server alive"""
    # Record the ping time
    ping_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Store the last ping time in app context
    if not hasattr(app, 'last_ping_time'):
        app.last_ping_time = []
    
    # Keep only the last 10 pings
    app.last_ping_time.append(ping_time)
    if len(app.last_ping_time) > 10:
        app.last_ping_time.pop(0)
